package com.muhmmad.cryptotracker.crypto.presentation.coin_details
...
fun LineChart(
    ...
    Canvas(modifier = modifier
        ...
        val viewPort = Rect(
            left = viewPortLeftX, top = viewPortTopY,
            right = viewPortRightX, bottom = viewPortBottomY
        )

       // drawRect(color = Color.Green.copy(0.3f), topLeft = viewPort.topLeft, size = viewPort.size)
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    start = Offset(x = x + result.size.width / 2f, y = viewPortBottomY),
                    end = Offset(x = x + result.size.width / 2f, y = viewPortTopY),
                    strokeWidth = if (selectedDataPointIndex == index) style.helperLinesThicknessPx * 1.5f else style.helperLinesThicknessPx
                )
            }
            ...
            if (selectedDataPointIndex == index) {
                ...
                val textPositionX =
                    if (selectedDataPointIndex == visibleDataPointIndices.last) {
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (showHelperLines) {
                drawLine(
                    color = style.unselectedColor,
                    start = Offset(x = viewPortLeftX, y = y + result.size.height.toFloat() / 2f),
                    end = Offset(x = viewPortRightX, y = y + result.size.height.toFloat() / 2f),
        ...
        for (i in 1 until drawPoints.size) {
            ...
            conPoints1.add(DataPoint(x, p0.y, ""))
            conPoints2.add(DataPoint(x, p1.y, ""))
        }
